    ...
    for i in xrange(ns):
        ...
        for p in cu: 
            for q in ucu: #p->q
                ...
                j = occu[1][str(joc)] # Epq
                for r in cu: #rrpq
                    sig21[i,:] += g_mtx[r*no+r,p*no+q] * C0[j,:] # Ia
                    sig21[:,i] += g_mtx[r*no+r,p*no+q] * C0[:,j] # Ib
                for r in cu: #pqrr
                    if r != p:
                        sig21[i,:] += g_mtx[p*no+q,r*no+r] * C0[j,:] # Ia
                        sig21[:,i] += g_mtx[p*no+q,r*no+r] * C0[:,j] # Ib
                    else:
                        sig21[i,:] += g_mtx[p*no+q,q*no+q] * C0[j,:] # Ia
                        sig21[:,i] += g_mtx[p*no+q,q*no+q] * C0[:,j] # Ib
                for r in ucu: #prrq
                    if r != q:
                        sig21[i,:] += g_mtx[p*no+r,r*no+q] * C0[j,:] # Ia
                        sig21[:,i] += g_mtx[p*no+r,r*no+q] * C0[:,j] # Ib
                for r in cu: #rqpr
                    if r != p:
                        sig21[i,:] += g_mtx[r*no+q,p*no+r] * C0[j,:] # Ia
                        sig21[:,i] += g_mtx[r*no+q,p*no+r] * C0[:,j] # Ib
                    ...
                    if r != p:
                        for s in ucu:
                            if s != q:
                                ...
                                j = occu[1][str(joc)] # Epq
                                sig21[i,:] += g_mtx[p*no+q,r*no+s] * C0[j,:] # Ia
                                sig21[:,i] += g_mtx[p*no+q,r*no+s] * C0[:,j] # Ib
    sig21 *= 0.5
    #==================
            ...
            for p in cu1:
                for r in cu2:
                    for s in ucu2:
                        ...
                        j = occu[1][str(joc)] # Epq
                        sig22[ia,ib] += g_mtx[p*no+p,r*no+s] * C0[ia,j]
            #ia!=ja,ib=jb
            for p in cu1:
                for q in ucu1:
                    for r in cu2:
                        ...
                        j = occu[1][str(joc)] # Epq
                        sig22[ia,ib] += g_mtx[p*no+q,r*no+r] * C0[j,ib]
            for p in cu1:
                for q in ucu1:
                    ...
                    for r in cu2:
                        for s in ucu2:
                            ...
                            sig22[ia,ib] += g_mtx[p*no+q,r*no+s] * C0[ja,jb]
